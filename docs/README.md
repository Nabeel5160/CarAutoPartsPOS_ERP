# Docs index — Car Auto Parts ERP

| Doc | Purpose |
|-----|---------|
| [GUIDE-BUSINESS.md](GUIDE-BUSINESS.md) | **Non-technical** — features & flows for owners / managers |
| [GUIDE-TECHNICAL.md](GUIDE-TECHNICAL.md) | **Technical** — architecture & code map for developers / tech leads |
| [PRODUCT-POSITIONING.md](PRODUCT-POSITIONING.md) | What we claim / do not claim; GTM language |
| [MASTER-ROADMAP.md](MASTER-ROADMAP.md) | Growth roadmap audit: DONE vs TODO toward 9.5+/10 |
| [ROADMAP-TO-TOP-TIER.md](ROADMAP-TO-TOP-TIER.md) | Climb to top-tier in-niche: Stages 0–5, Phase 12+ backlog |
| [PHASE-COMPLETION-PLAN.md](PHASE-COMPLETION-PLAN.md) | Finish remaining Phases 14–21: reconcile parallel work, sequence, DoD |
| [CHANGELOG-ENTERPRISE.md](CHANGELOG-ENTERPRISE.md) | Shipped enterprise phases (0–11+) |
| [QA-PROGRAM-ABC.md](QA-PROGRAM-ABC.md) | QA notes for Programs A / B / C1 |
| [ANIMATED-CHARTS.md](ANIMATED-CHARTS.md) | Apache ECharts CapChart / timeline / bar3D |
| [VERTICAL-PROFILES.md](VERTICAL-PROFILES.md) | Auto / bike / retail config profiles |
| [CLIENT-REPORTS-ROADMAP.md](CLIENT-REPORTS-ROADMAP.md) | Client reports A–C + remaining gaps |
| [PERFORMANCE.md](PERFORMANCE.md) | Phase 19 latency budgets, indexes, smoke measurement |
| [PAGINATION-PLAN.md](PAGINATION-PLAN.md) | Grid inventory, paging classification, W0–W5 implementation waves |
| [COUNTER-UX-POLISH-LOOP.md](COUNTER-UX-POLISH-LOOP.md) | Counter UX polish waves W0–W5 (login→sell friction, POS feel) |
| [CRM-LOOP.md](CRM-LOOP.md) | Light CRM waves W0–W5 (leads → activities → pipeline → 360) |
| [SLA-FLOWS.md](SLA-FLOWS.md) | **Complete SLA flows** — tickets, multi-pipeline, ops clocks, UI/API |
| [SLA-LOOP.md](SLA-LOOP.md) | Service SLA W0–W5 implementation loop |
| [SLA-COMPLETE-LOOP.md](SLA-COMPLETE-LOOP.md) | SLA productization waves |
| [SLA-EXPANSION.md](SLA-EXPANSION.md) | Multi-pipeline + thin ops clocks checklist |
| [FBR-INTEGRATION.md](FBR-INTEGRATION.md) | FBR Digital Invoicing — full flow & what is implemented |
| [FBR-PRODUCTION.md](FBR-PRODUCTION.md) | FBR sandbox → production cutover playbook |
| [DEPLOYMENT.md](DEPLOYMENT.md) | Install, LAN, FBR, production runbook |
| [COSTING.md](COSTING.md) | Average / FIFO valuation notes |
| [COA-SEED.md](COA-SEED.md) | Chart of accounts seed |
| [LOCALIZATION.md](LOCALIZATION.md) | EN/UR locale |
| [SSO-HOOKS.md](SSO-HOOKS.md) | SSO extension points |
| [adr/](adr/) | Architecture decision records |
| [SMOKE-INTEGRATION-PLAN.md](SMOKE-INTEGRATION-PLAN.md) | Phased API/UI smoke waves A–J, pass criteria, scripts |
| [SMOKE-ENDPOINT-INVENTORY.md](SMOKE-ENDPOINT-INVENTORY.md) | METHOD/path/controller/auth inventory from Api controllers |
| [SMOKE-RESULTS.md](SMOKE-RESULTS.md) | Timestamped smoke run summaries + nav↔API notes |
